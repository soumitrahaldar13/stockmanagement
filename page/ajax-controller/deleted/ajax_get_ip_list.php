...
// check emp reg or not
    $CPU_count = mysqli_query($con, "SELECT * FROM `user` WHERE `emp_id`='$emp_code'") or die('Error');
         $count = mysqli_num_rows($CPU_count);

if ($count == 0) {
    //echo 0;die;
    ?>
        <h4>User doesn't exists</h4>
    <?php
    } else {
...
?>
    ...
    <select id='ip-dropdown' name="ip-dropdown" required>
