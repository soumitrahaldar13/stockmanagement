...
// echo "INSERT INTO `cpu`(`id`, `GEM_NO`, `GEM_Date`, `VENDOR_NAME`, `BRAND_NAME`, `Serial_Number`, `Model_No`, `Delivery_Date`, `Problem`, `START_DATE`, `END_DATE`, `Defunct`, `is_amc`, `ewaste`, `SSD_Capacity`, `HDD_Capacity`, `Processor`, `Processor_Base_Frequency`, `RAM`, `Optical_Drive`, `Graphic_Card`, `Wireless`, `OS`, `OS_Serial_no`, `ChipSet_Number`, `upgrade`, `active`, `select_item`, `monitor_size`) VALUES ('','$GEM_NO','$GEM_Date',' $VENDOR_NAME','$BRAND_NAME','$Serial_Number','$Model_No','$text','$Problem1','$START_DATE1','$END_DATE1','0','0','0','$SSD_Capacity','$HDD_Capacity','$Processor','$Processor_Base','$ram','$Optical_Drive','$GCD','$Wireless','$os','$os_No','$ChipSet_Number','0','0','$item','$Monitor_Size')";die;
  ...
  $query = $con->query("INSERT INTO `laptop`(`id`, `GEM_NO`, `GEM_Date`, `VENDOR_NAME`, `BRAND_NAME`, `Serial_Number`, `Model_No`, `Delivery_Date`, `Problem`, `START_DATE`, `END_DATE`, `Defunct`, `is_amc`, `ewaste`, `SSD_Capacity`, `HDD_Capacity`, `Processor`, `Processor_Base_Frequency`, `RAM`, `Optical_Drive`, `Graphic_Card`, `Wireless`, `OS`, `OS_Serial_no`, `ChipSet_Number`, `upgrade`, `active`, `monitor_size`) VALUES ('','$GEM_NO','$GEM_Date',' $VENDOR_NAME','$BRAND_NAME','$Serial_Number','$Model_No','$text','$Problem1','$START_DATE1','$END_DATE1','0','0','0','$SSD_Capacity','$HDD_Capacity','$Processor','$Processor_Base','$ram','$Optical_Drive','$GCD','$Wireless','$os','$os_No','$ChipSet_Number','0','0','$Monitor_Size')");
  if ($query) {
    echo "<script>alert('update laptop Sucessfully');window.location.href='admin_laptop.php'; </script>";
...
<section class="home-section">
  <nav>
    <div class="sidebar-button">
      ...
      <span class="dashboard">LAPTOP</span>
    </div>
  ...
  <div class="home-content">
    ...
    <div class="sales-boxes">
      <div class="recent-sales box" style="width: 95%;">
        <div class="title">STOCK ALL laptop's </div>
        ...
        <div class="table-responsive box">
          ...
          <!-- <form method="post" action=""> -->
            <table cellpadding="0" cellspacing="0" border="0" class="table table-condensed" id="example">
              ...
              <tbody>
                <?php
                $query = mysqli_query($con, " SELECT * FROM `laptop` WHERE `ewaste`='0' AND `active`= '0' ORDER BY `id` DESC") or die(mysqli_error($con));
                // echo 1;die;
                $counter = 0;
                ...
                ?>
                  ...
                  <tr>
                    ...
                    <td><a href="admin_laptop_AMC.php?id=<?php echo $row['id']; ?>&AMC=<?php echo $row['is_amc']; ?>"><?php if ($row['is_amc'] == 0) { ?><span class="glyphicon glyphicon-unchecked" style="font-size:20px; color:red;">N</span> <?php } else { ?><span class="glyphicon glyphicon-unchecked" style="font-size:20px; color:GREEN;">Y</span><?php } ?></span></a></td>
                    ...
                    <td><?php if ($row['Defunct'] == 0 ) {  }else { ?><a href="admin_LAPTOP_ewaste.php?id=<?php echo $row['id']; ?>"><span class="glyphicon glyphicon-send" style="font-size:20px; color:blue;"></span></a> <?php } ?> </td>
                    
                   <td disabled><?php if ($row['upgrade'] == 0) {}else{?><a href="Call_Log_upgrade.php?S_N=<?php echo $row['Serial_Number'];?>&id=<?php echo $row['id'];?>&PRODUCT=laptop"><span class="glyphicon glyphicon-send" style="font-size:20px; color:blue;"></span></a> <?php }?></td>

                   
                  <?php
                  include 'admin_laptop_modify.php';
                }
...
$(document).ready(function(){
    $('#Serial_Number').keyup(function() {
      ...
      $.ajax({
        type: 'POST',
        url: 'admin_laptop-SLNO_check.php',
